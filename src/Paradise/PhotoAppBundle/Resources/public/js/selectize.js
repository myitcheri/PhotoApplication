	...
	*/
	...
	Selectize.registerPlugin('remove_button', function(options) {
		...
		this.settings.render.item = function(data) {
			...
			var catchOption = data[self.settings.optgroupField];
			var label = data[self.settings.labelField];
                        
                        var optionData = catchOption.split(',');
                        var category = optionData[0];
                        var userID = optionData[1];
                        
                        var PARADISE_USERID = $('#user_id').val();
                        
                        if(category===null || category===false){ category = ''; } //Hack to add tag category style
                        
                        if(userID === PARADISE_USERID || userID === 'enableDelete')
                        {
                            return '<div class="item '+ category +'">' + label +' <a href="javascript:void(0)" class="remove" tabindex="-1" title="Remove">&times;</a></div>';
                        }
                        else if(userID !== PARADISE_USERID)
                        {

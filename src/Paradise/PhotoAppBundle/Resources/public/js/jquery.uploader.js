$(document).ready(function() {
	...
	$('#drop-files').bind('drop', function(e) {
		...
		$('#file-holder').hide();
                ...
                $('#uploaded-files-id').html('');
                
                var upload_desc = $('#upload-desc').val();
		...
		$.each(files, function(index, file) {
			...
			if (!files[index].type.match('image.*')) {
				...
				if(errMessage == 0) {
					$('#drop-files').html('Only image(s) are allowed');
				}
				return false;
			}
			...
			var fileReader = new FileReader();
				...
				fileReader.onload = (function(file) {
					...
					return function(e) { 
						...
						dataArray.push({name : file.name, value : this.result, filesize: file.size, filetype: file.type, uniqueName: randomString+file.name, desc: upload_desc });
	...
	$('#upload-button .upload').click(function() {
		...
		$('#uploaded-files').html('');
                ...
                if($('#upload-desc').val()){
                     $('#upload-desc').hide();
                     $('#photodesc_content').show();
                     $('#photodesc_content').html( $('#upload-desc').val() );
                }
		...
		$.each(dataArray, function(index, file) {	
			...
			$.post($(location).attr('href')+'/save', dataArray[index], function(data) {
				...
				if(totalPercent*(x) == 100) {
					...
					$('#loading-content').html('Uploading '+fileName);
                                ...
                                $('#uploaded-files').append('<div style="float:left; margin:3px;"><img src="'+dataArray[index].value+'" width="211"/></div>');
                                $('#uploaded-files-id').append(dataArray[index].uniqueName+'|');
			});
		});
